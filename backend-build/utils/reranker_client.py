...
RERANKER_TIMEOUT = float(os.getenv("RERANKER_TIMEOUT", "120.0"))
...
class SyncRerankerClient:
    ...
    def rerank(
        ...
        with httpx.Client(base_url=self.base_url, timeout=self.timeout) as client:
            ...
            doc_list = []
            for doc in documents:
                # If document already has metadata key, use it directly
                if 'metadata' in doc:
                    doc_list.append({
                        "text": doc.get(text_key, ''),
                    ...
                    # Otherwise, put all non-text fields in metadata
                    doc_list.append({
                        "text": doc.get(text_key, ''),
                        "metadata": {k: v for k, v in doc.items() if k != text_key}
                    })
